...
model = LinearRegression()
cv_result = cross_validate(model, X, y, scoring="neg_mean_squared_error", cv=5)
...
Next, construct the grid search. The argument `cv` is specified in the same way as in ordinary `cross-validation`. The argument `refit=True` controls whether the model is refit on the entire training set using the best hyperparameters. The default value is `True`.
...
6. The fitted model can then be used to generate predictions.
...
subgraph C[Categorical Feature: group]
direction TB
C1[OneHotEncoder]
end
...
cv_result = cross_validate(
    pipe, X, y, cv=KFold(5, shuffle=True, random_state=1), scoring=["r2", "neg_mean_squared_error"]
)
