...
There are two ways to build more complicated models in Python. The first is to directly modify the feature matrix `X`. For example, assuming we are adding the squared term of `x2`.
...
Another way to generate a more complicated model is to use `PolynomialFeatures` from `sklearn.preprocessing`, which can automatically create higher-degree terms and interaction terms. This is especially useful when there are many predictors and it becomes tedious to write all transformed terms by hand.
...
We can then use the resulting matrix as the feature matrix in a regression model.
:::
