...
In traditional regression to examine diagnostic plots to detect potential problems in the model is a powerful tool. However statistical learning primarily focuses on prediction. Therefore diagnostics is not our primary focus, and this section is optional.
...
We will compute some qualities for the residual analysis. Although we mainly focus on `sklearn`, in order to perform residual analysis, we need help from `statsmodels`. Therefore we need a `statsmodels` model to hold the data. Since we already have the feature matrix and the target maxtrix, the fastest way to use `statsmodels` is through its `statsmodels.api` (comparing to the R style routine `statsmodels.formula.api`).
...
The fitted values, the residuals as well as leverage, cook's distance etc. can be directly extracted from this `model` object. We wrap them into DataFrames for plots. 

```{python}
y_pred = model.fittedvalues
res = model.resid

model_output = pd.DataFrame({"y_true": y, "y_pred": y_pred, "res": res})
```
...
The default `plot_ccpr()` doesn't show the smooth curve. If you need it, you have to use `lowess()` function to compute the curve and draw it manually.
...
The same trick applies in the grid case.
...
::: {.callout-note}
## 
In statistical learning (which is implemented mainly by `sklearn`), we focus on predictive performance, therefore models are typically built by constructing a feature matrix. It is usually not very convienent to make some modifications of the model.

In contrast, the traditional regression (which is implemented mainly by `statsmodels`) allows explicit specification of statistical models through a formula language, which is convenient when precise control of model terms and statistical inference are required.
:::

Before any tricks, we could always introduce new variables by modifying the original dataset.
...
We are adding the squared term of $x_2$ and the interaction term $x_1x_2$ to $X$ to form the new feature matrix `X2`. We could then use `X2` to fit a new model.
...
::: {.callout-note collapse="true"}
## Categorical data (`pandas` version)

We could use `pandas` to directly modify the original dataset.
```{python}
...
## Categorical data (`sklearn` version)
...
It will convert each column into dummy variables if you have multiple rows. In the below example the first 5 columns belong to `x` and the last 2 columns belong to `group`. You may compare the result with the `pd.get_dummies()` version.
...
::: {.callout-note collapse="true"}
## Standarlize, centrailize and normalize
...
In order to only choose some columns to transform, we may use `ColumnTransformer`.
...
```
Note that you may switch the order of columns when constructing `ColumnTransformer`.
