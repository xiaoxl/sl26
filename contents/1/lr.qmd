...
::: {.callout-note collapse="true"}
## Categorical variables (`pandas` version)
...
## Categorical variables (`sklearn` version)
...
In `sklearn`, there are several ways to perform $k$-fold cross-validation. From `KFold` to `cross_validate` and then to `cross_val_score`, the interface becomes easier to use, but also less flexible.

::: {.callout-note collapse="true"}
## `KFold`
`KFold` from `sklearn.model.selection` is used to split the dataset into $k$ groups, called folds. In each iteration, one fold is used as the validation set and the remaining folds are used as the training set.
