...
Lasso regression modifies the OLS objective by adding an $L_1$ penalty.
...
The lasso penalty shrinks the coefficients toward zero. Unlike ridge regression, however, lasso can set some coefficients **exactly equal to zero**. Therefore, lasso performs both **shrinkage** and **variable selection**.

---

### Alternative formulation

By the theory of Lagrange multipliers, lasso can also be written in constrained form:
...
Here $s \ge 0$ controls the size of the coefficient vector.

- Large $s$ corresponds to small $\lambda$, so the estimator is close to OLS.
- Small $s$ corresponds to large $\lambda$, so the coefficients are forced toward zero.

This formulation is useful because it makes the geometry of lasso easier to understand.

---

### Geometry and sparsity

The geometric reason that lasso produces sparse solutions is that its constraint region has corners.

- Ridge regression uses an $L_2$ constraint region, which is circular in two dimensions.
- Lasso uses an $L_1$ constraint region, which is diamond-shaped in two dimensions.

Because the diamond has sharp corners on the coordinate axes, the minimum often occurs at a corner. At such a point, one or more coefficients are exactly zero. This is the geometric explanation for the variable-selection property of lasso.

::: {.callout-note}
## Ridge versus Lasso

Both ridge and lasso shrink coefficients toward zero.

- **Ridge** uses an $L_2$ penalty and usually keeps all coefficients nonzero.
- **Lasso** uses an $L_1$ penalty and may set some coefficients exactly to zero.

Therefore, ridge is mainly a shrinkage method, while lasso is both a shrinkage method and a variable-selection method.
:::
...
A model with intercept can be rewritten in no-intercept form by centering the response and the predictors:
...
After centering, the intercept becomes zero automatically, so the penalty only needs to be applied to the slope coefficients.

::: {.callout-note}
## Standardization in Practice

Because the lasso penalty depends on the magnitudes of the coefficients, lasso is usually implemented after standardizing the predictors. This puts all predictors on a comparable scale, so the penalty treats them fairly.

In practice,

- predictors are typically **standardized**,
- the response is typically **centered** but not standardized.
...
In `scikit-learn`, `Lasso` and `LassoCV` use coordinate descent to fit the model. The argument `alpha` is the regularization parameter, and `fit_intercept=True` means the intercept is handled separately. If `fit_intercept=False`, the data is expected to be centered beforehand.
:::
...
lasso_pipe = Pipeline([("scaler", StandardScaler()), ("model", Lasso(alpha=0.1))])
