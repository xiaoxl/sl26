...
<!-- At step $k$, we work with residual matrices:

- $X_k$: predictor residuals  
- $y_k$: response residuals   -->
...
Similar to PCR, the max number of components is again the minimum between the number of rows and the number of columns of `X_train`. 
...
```{python}
y_pred_pls = grid.predict(X_test)
...
- PLSRegression automatically centers and scales (unless disabled)
- Unlike PCA: uses both $X$ and $y$
- Number of components: choose via cross-validation
:::
